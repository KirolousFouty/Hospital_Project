...
void AStar::tracePath(cell cellDetails[][COL], Pair dest)
{
    ...
    while (!Path.empty())
    {
        ...
        Path.pop();
        this->result.append(" -> (" + QString::number(p.first) + "," + QString::number(p.second) + ")");
    }
...
void AStar::aStarSearch(int grid[][COL], Pair src, Pair dest)
{
    ...
    {
        this->result.append("Source is invalid\n");
        return;
    }
    ...
    {
        this->result.append("Destination is invalid\n");
        return;
    }
    ...
    {
        this->result.append("Source or the destination is blocked\n");
        return;
    }
    ...
    {
        this->result.append("We are already at the destination\n");
        return;
    }
    ...
    while (!openList.empty())
    {
        ...
        if (isValid(i - 1, j) == true)
        {
            ...
            if (isDestination(i - 1, j, dest) == true)
            {
                ...
                this->result.append("The destination cell is found\n");
    ...
    // blockages)
    if (foundDest == false)
        this->result.append("Failed to find the Destination Cell\n");
